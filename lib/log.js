...
function Log(options) {
  ...
  this.info.name = options.name || process.title;
  this.info.hostname = options.hostname || Os.hostname();
  this.info.version = options.version || 0;
  this.info.pid = options.pid || process.pid;
  this.transports = [];
  this.serializers = {};
  this.levels = {};
...
Log.prototype.child = function(childinfo) {
  var parent = this;
  var child = new Log(this.info);
  child.log = function(level) { return parent.log.apply(parent, [childinfo].concat(Array.prototype.slice.call(arguments, 1))); };
  child.send = function(evt) { return parent.send(evt); };
  child.transport = function(transport) { return parent.transport(transport); };
